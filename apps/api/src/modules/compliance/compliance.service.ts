...
export class ComplianceService {
  ...
  async exportCisEvidencePack(
    ...
    const allChecks = (report.agents || []).flatMap((agent) => agent.checks || []);
    ...
    const evidenceHash = crypto
      .createHash('sha256')
      .update(JSON.stringify({ tenantId, report }))
      .digest('hex');
    ...
    doc.text(`Tenant: ${tenant?.name || tenantId}`);
    ...
    doc.text(
      `Checks: ${passCount} pass / ${failCount} fail / ${warningCount} warning`,
    );
    doc.moveDown();
    ...
    doc
      .fontSize(7)
      .fillColor('#666')
      .text(`SHA-256 evidence digest: ${evidenceHash}`, {
        align: 'center',
      });

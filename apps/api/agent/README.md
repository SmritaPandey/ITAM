...
```bash
# Create a plist file
cat > ~/Library/LaunchAgents/com.qs.discovery-agent.plist << EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key><string>com.qs.discovery-agent</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/bin/node</string>
    <string>/path/to/qs-discovery-agent.js</string>
...
EOF
launchctl load ~/Library/LaunchAgents/com.qs.discovery-agent.plist
```

### Linux (systemd)
```bash
sudo cat > /etc/systemd/system/qs-discovery-agent.service << EOF
[Unit]
Description=QS Discovery Agent
After=network.target

[Service]
ExecStart=/usr/bin/node /opt/qs-agent/qs-discovery-agent.js --server http://192.168.1.50:4100 --user staff@acme.com --pass Staff@123
Restart=always
RestartSec=10
...
EOF
sudo systemctl enable qs-discovery-agent
sudo systemctl start qs-discovery-agent
```

### Windows (Task Scheduler)
1. Open Task Scheduler
2. Create Basic Task → "QS Discovery Agent"
3. Trigger: At logon
4. Action: Start a program
   - Program: `node.exe`
   - Arguments: `C:\path\to\qs-discovery-agent.js --server http://192.168.1.50:4100 --user staff@acme.com --pass Staff@123`

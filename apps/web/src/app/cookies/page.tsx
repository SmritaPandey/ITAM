  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, color: txt, fontFamily: "'Plus Jakarta Sans', 'Inter', system-ui, sans-serif", transition: 'background 0.5s, color 0.5s' }}>
      ...
      <div style={{ maxWidth: 840, margin: "0 auto", padding: "64px 24px 120px", paddingTop: 80, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ background: card, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: `1px solid ${border}`, borderRadius: 20, padding: "40px 48px", display: "flex", flexDirection: "column", gap: 32, lineHeight: 1.8, fontSize: 14, color: "#cbd5e1" }}>
          ...
          <Section title="1. What Are Cookies & Session Identifiers?">
            ...
            QS Asset Management implements modern first-party browser <code style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.15)", padding: "1px 6px", borderRadius: 4, color: "#22d3ee" }}>localStorage</code> mechanisms in place of traditional tracking cookies for nearly all active system functions.
          </Section>
          ...
          <Section title="3. Mandatory Essential Storage (Always Active)">
            ...
            <ul style={{ margin: "12px 0 0", paddingLeft: 20, fontSize: 13, display: "flex", flexDirection: "column", gap: 8 }}>
              <li>
                <code style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.15)", padding: "2px 6px", borderRadius: 4, color: "#22d3ee" }}>accessToken</code> — Encrypted JSON Web Token validating your administrative session authorization.
              </li>
              <li>
                <code style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.15)", padding: "2px 6px", borderRadius: 4, color: "#22d3ee" }}>refreshToken</code> — Cryptographic token used to securely extend authentication sessions without prompting credential re-entry.
              </li>
              <li>
                <code style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.15)", padding: "2px 6px", borderRadius: 4, color: "#22d3ee" }}>theme</code> — Caches user color style preferences (dark/light mode configuration).
              </li>
              <li>
                <code style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.15)", padding: "2px 6px", borderRadius: 4, color: "#22d3ee" }}>qs_cookie_consent</code> — Stores user preference configurations for optional analytical trackers.
              </li>
            </ul>
          </Section>
          ...
          <Section title="4. Optional Analytics Storage (Consent Required)">
            ...
            <div style={{ marginTop: 12, padding: "12px 16px", borderRadius: 10, background: "rgba(16,185,129,0.03)", border: "1px solid rgba(16,185,129,0.15)", color: "#10b981", fontSize: 13 }}>
              🔒 <strong style={{ color: "#a7f3d0" }}>Privacy Guarantee:</strong> We explicitly DO NOT track keyboard input contents, configuration parameter values, physical device names, password lengths, or any system configurations inside our analytical trackers.
            </div>
          </Section>
          ...
          <Section title="8. Appointed Contact Details">
            ...
            <div style={{ marginTop: 12, padding: 18, borderRadius: 12, background: "rgba(255,255,255,0.01)", border: `1px solid ${border}`, fontSize: 13 }}>
              ✉️ <strong style={{ color: "#f1f5f9" }}>Direct Email Desk:</strong> privacy@neurqai.com
            </div>
          </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 8 }}>
      <h2 style={{ fontSize: 18, fontWeight: 800, color: "#f1f5f9", marginBottom: 12, letterSpacing: "-0.02em" }}>{title}</h2>
      <div>{children}</div>
    </div>
  );
}
...
function Table({ data }: { data: { type: string; purpose: string; retention: string; optional: string }[] }) {
  return (
    <div style={{ overflowX: "auto", margin: "16px 0", borderRadius: 12, border: "1px solid rgba(42,49,80,0.4)" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 500 }}>
        <thead>
          <tr style={{ borderBottom: "1px solid rgba(42,49,80,0.5)", background: "rgba(255,255,255,0.02)" }}>
            {["Storage Category", "Operational Purpose", "Retention Window", "Consent Requirement"].map(h => (
              <th key={h} style={{ padding: "12px 16px", textAlign: "left", fontWeight: 700, color: "#06b6d4", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {data.map((r, i) => (
            <tr key={r.type} style={{ borderBottom: i === data.length - 1 ? "none" : "1px solid rgba(42,49,80,0.3)", background: "transparent" }}>
              <td style={{ padding: "14px 16px", fontWeight: 700, color: "#f1f5f9" }}>{r.type}</td>
              <td style={{ padding: "14px 16px", color: "#cbd5e1" }}>{r.purpose}</td>
              <td style={{ padding: "14px 16px", color: "#94a3b8" }}>{r.retention}</td>
